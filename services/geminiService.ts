...
export const generateSoccerUniform = async (
  ...
  try {
    ...
    // Branch logic based on the selected model
    if (options.render.model === 'Model: High Quality') {
        ...
        if (response.generatedImages && response.generatedImages.length > 0) {
            ...
            return { imageUrl, text: prompt };
             ...
             return { imageUrl: null, text: 'No image generated by high quality model.' };
        }
    } else { // 'Model: Standard'
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash-image',
    ...
    throw new Error("An unknown error occurred during the soccer uniform generation process.");
  }
};
